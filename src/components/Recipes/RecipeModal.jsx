    ...
    if (!ingredient.measurementQuantity.quantity) {
      return (
        <li key={`${modalData.title}${i}`}>
          {!ingredient.measurementUnit.unit === "null"
    ...
    <div className="fixed flex justify-center z-20 recipeCard h-[88vh] w-[80vw] snap-center overflow-y-auto top-[6vh] left-[10vw] bottom-[6vh] right-[10vw]">
      <div className="flex flex-col gap-5 p-3">
        ...
        <div className="flex items-center justify-between">
          <img
            src={`${modalData.image}`}
            className="h-32 w-36 rounded-[5px] m-1 self-start lg:self-center"
          />
          <div className="grow ms-1 self-center lg:self-end">
            <h1 className="text-lg lg:text-2xl hyphens-auto">
              {modalData.title}
            </h1>
            <h3 className="mt-3 sm:mt-7 lg:my-7 text-sm lg:text-base">
        ...
        <ul className="list-disc ml-5 columns-2 text-sm lg:text-base">
          {ingredients}
        </ul>
